...
def test_json_parser_parses_dict_concat():
    parser = JSONParser()
    with patch("builtins.open", mock_open(read_data="{}")), patch("json.load", return_value={"a": 1}):
...
def test_json_parser_parses_list_no_concat():
    ...
    data = [{"a": 1}, {"b": 2}]
    with patch("builtins.open", mock_open(read_data="[]")), patch("json.load", return_value=data):
        result = parser.parse_file(Path("t.json"))
    assert result == data
...
def test_json_parser_row_joiner_config():
    ...
    with patch("builtins.open", mock_open(read_data="[]")), patch("json.load", return_value=[{"a": 1}, {"b": 2}]):
        result = parser.parse_file(Path("t.json"))
...
def test_json_parser_forwards_json_config():
    ...
    with patch("builtins.open", mock_open(read_data="[]")), patch("json.load", return_value=[]) as mock_load:
